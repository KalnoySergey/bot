...
class Mailer:
    ...
    def __init__(self, token):
        ...
        self.capchaButton = InlineKeyboardBuilder()
        self.capchaButton.button(text=f"Підтвердити ✅", callback_data='confirm')
        self.WelcomesList = []
    ...
    async def chat_join_request_handler(self, chat_join_request: ChatJoinRequest, bot: Bot):
        ...
        if await BotDB.capcha_is_active(bot_id):
            if not await BotDB.user_exist(bot_id, user_id):
                await bot.send_message(user_id, cap, parse_mode='Markdown', reply_markup=self.capchaButton.as_markup())
        if self.join:
    ...
    async def confirm(self, call: CallbackQuery, bot: Bot):
        await call.answer()
        me = await bot.get_me()
        bot_id = me.id
        if not await BotDB.user_exist(bot_id, call.from_user.id):
            ...
            for welcome in mybot.WelcomesList:
                ...
                if delay_time == 'now':
                    ...
                    h = int(delay_time.split(':')[0])
                    m = int(delay_time.split(':')[1])
                    s = int(delay_time.split(':')[2])
    ...
    async def send_message(bot_name, welcome, user):
        ...
        welcome_id = await BotDB.get_welcome_id_by_name(welcome, bot_id)
        text = await BotDB.get_welcome_text(welcome_id)
        button = bot.WelcomesDict.get(welcome)
        builder = button.buttons
        mes = await bot.mybot.send_message(user, text, parse_mode='Markdown', reply_markup=builder.as_markup())
        if await BotDB.welcome_del_is_active(welcome_id):
            delete_time = await BotDB.get_welcome_delete(welcome_id)
            h = int(delete_time.split(':')[0])
            m = int(delete_time.split(':')[1])
            s = int(delete_time.split(':')[2])
...
class Manager:
    ...
    async def token_add(message: Message, state: FSMContext):
        ...
        mes = await message.answer(f"Бот успішно доданий")

        scheduler = AsyncIOScheduler()
        scheduler.add_job(Manager.dele, trigger='date', run_date=datetime.now() + timedelta(seconds=30), kwargs={'message': mes})
        scheduler.start()
        ...
        dp2.callback_query.register(bot.confirm, F.data.endswith('confirm'))
    ...
    async def new_welcome_button(message: Message, state: FSMContext):
        ...
        mes1 = await message.answer(f'Введіть затримку відправки у форматі HH:MM:SS\n'
    ...
    async def new_welcome_delay(message: Message, state: FSMContext):
        ...
        if newtext == '0':
            ...
            mes = data.get('todelete')
            mes1 = await message.answer(f'Введіть час, через який видалиться привітання у форматі HH:MM:SS\n'
                                        f'Щоб не видаляти привітання введіть 0', parse_mode='Markdown')
            await mes.delete()
            await state.update_data(bot_name=bot_name, newwelcome=welcome, todelete=mes1)
            await state.set_state(MyStates.NEW_WELCOME_DELETE)
        elif time_pattern.match(newtext):
            ...
            mes = data.get('todelete')
            mes1 = await message.answer(f'Невірний формат, ще раз введіть затримку відправки у форматі HH:MM:SS\n'
                                        f'Щоб відправити одразу введіть 0', parse_mode='Markdown')
            await mes.delete()
            await state.update_data(bot_name=bot_name, newwelcome=welcome, todelete=mes1)
    async def new_welcome_delete(message: Message, state: FSMContext):
        ...
        if newtext == '0':
            ...
            builder = InlineKeyboardBuilder()
            builder.button(text=f"Продовжити", callback_data=f"{bot_name}*welcomes")
            ...
            mes = data.get('todelete')
            mes1 = await message.answer(f'Невірний формат, ще раз введіть час, через який видалиться привітання у форматі HH:MM:SS\n'
                                        f'Щоб не видаляти привітання введіть 0', parse_mode='Markdown')
            await mes.delete()
            await state.update_data(bot_name=bot_name, newwelcome=welcome, todelete=mes1)
    async def capcha(call: CallbackQuery, state: FSMContext):
        ...
        message2 = await call.message.answer(cap, parse_mode='Markdown', reply_markup=bot.capchaButton.as_markup())
        ...
        if await BotDB.admin_exist(admin_id):
            if await BotDB.admin_get_role(admin_id) == 'Admin':
                if await BotDB.check_subscription_status(bot_id):
                    ...
                    await call.message.edit_text(f"Підписка закінчилась\n\nЗверніться до @SlL0Nn для її продовження", parse_mode='Markdown', reply_markup=builder.as_markup())
    ...
    async def welcome_delay_change(call: CallbackQuery, state: FSMContext):
        ...
        mes1 = await call.message.edit_text(f'Введіть нову затримку відправки у форматі HH:MM:SS\n'
                                    f'Щоб відправити одразу введіть 0', parse_mode='Markdown')
        mes = data.get('todelete')
        await mes.delete()
    ...
    async def welcome_delay_change2(message: Message, state: FSMContext):
        ...
        if newtext == '0':
            ...
            mes1 = await message.answer('Невірний формат, ще раз введіть нову затримку відправки у форматі HH:MM:SS\n'
                                    f'Щоб відправити одразу введіть 0', parse_mode='Markdown')
            await mes.delete()
            await state.update_data(todelete=mes1, bot_name=bot_name, welcome=welcome)
    async def welcome_delete_change(call: CallbackQuery, state: FSMContext):
        ...
        mes1 = await call.message.edit_text(f'Введіть новий час, через який видалиться привітання у форматі HH:MM:SS\n'
                                    f'Щоб не видаляти привітання введіть 0', parse_mode='Markdown')
        mes = data.get('todelete')
        await mes.delete()
    ...
    async def welcome_delete_change2(message: Message, state: FSMContext):
        ...
        if newtext == '0':
            ...
            mes1 = await message.answer('Невірний формат, ще раз введіть новий час, через який видалиться привітання у форматі HH:MM:SS\n'
                                    f'Щоб не видаляти привітання введіть 0', parse_mode='Markdown')
            await mes.delete()
            await state.update_data(todelete=mes1, bot_name=bot_name, welcome=welcome)
    ...
    async def new_spam_button(message: Message, state: FSMContext):
        ...
        mes1 = await message.answer(f'Введіть час, коли буде зроблено розсилку у форматі HH:MM:SS\n'
                             f'Щоб відправити одразу введіть 0', parse_mode='Markdown')
        mes = data.get('todelete')
        await mes.delete()
        await state.update_data(bot_name=bot_name, todelete=mes1, spam_id=spam_id)
    ...
    async def new_spam_delay(message: Message, state: FSMContext):
        ...
        if newtext == '0':
            ...
            mes = data.get('todelete')
            mes1 = await message.answer(f'Введіть час, через який видалиться розсилка у форматі HH:MM:SS\n',
                                        parse_mode='Markdown')
            await mes.delete()
            await state.update_data(bot_name=bot_name, todelete=mes1, spam_id=spam_id)
            ...
            mes = data.get('todelete')
            mes1 = await message.answer(f'Невірний формат, ще раз введіть час, коли буде зроблено розсилку у форматі HH:MM:SS\n',
                                        parse_mode='Markdown')
            await mes.delete()
            await state.update_data(bot_name=bot_name, todelete=mes1, spam_id=spam_id)
    async def new_spam_delete(message: Message, state: FSMContext):
        ...
        if time_pattern.match(newtext):
            ...
            mes1 = await message.answer(f'Невірний формат, ще раз введіть час, через який видалиться розсилка у форматі HH:MM:SS', parse_mode='Markdown',)
            mes = data.get('todelete')
            await mes.delete()
            await state.update_data(bot_name=bot_name, todelete=mes1, spam_id=spam_id)
